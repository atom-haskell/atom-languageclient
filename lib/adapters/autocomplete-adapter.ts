...
export default class AutocompleteAdapter {
  ...
  public async getSuggestions(
    ...
    if (
      ...
      triggerChar === '' &&
      minimumWordLength > 0 &&
      request.prefix.length < minimumWordLength
    ) {
      return [];
    }
      ...
      const completions = await Utils.doWithCancellationToken(
        ...
        (cancellationToken) =>
          server.connection.completion(
            AutocompleteAdapter.createCompletionParams(request, triggerChar),
            cancellationToken,
          ),
      );
  ...
  public static createCompletionParams(request: AutocompleteRequest, triggerCharacter: string): CompletionParams {
    return {
  ...
  public completionItemsToSuggestions(
    ...
    return new Map(
      (Array.isArray(completionItems) ? completionItems : completionItems.items || [])
        ...
        .map<[AutocompleteSuggestion, [CompletionItem, boolean]]>((s) => [
